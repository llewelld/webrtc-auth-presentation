...
\item Once the key is established it's used to establish a secure SRTP and SRTCP stream between them. These can transfer any data, but the obvious types are video and audio data.
\end{enumerate}
}
...
However, we want to provide a method that allows sessions to be established between devices in a way that prevents the third-party service tracking the user or violating the security.s
}
...
\includegraphics[width=1.0\textwidth]{pairing}
\end{column}
\begin{column}[T]{0.5\textwidth}
\includegraphics[width=1.0\textwidth]{paired}
\end{column}
\end{columns}
...
From the user's point of view, they run a javascript app on both devices. On one device the javascript displays a QR code. On the other it provides a scanner the user scans it with.

From the user's perspective this completes the process. The photos are showing the yopet.us implementation.
}
...
Use a third-party signalling service for message passing (e.g. respoke.io)
...
We have to make some reasonable assumptions about the security of the QR-code channel in step 3. In particular, our thread model assumes the two devices are trusted (not compromised) and that the Javascript sent to the browser can be trusted (e.g. it can be checked on the device).
}
